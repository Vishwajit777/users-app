    ...
    <div className={styles.deleteUserWrapper}>
      ...
      <span className={styles.crossIcon} onClick={()=>setIsOpen(false)}><Cross /></span>
        <div className={styles.container}>
          <h1 className={styles.title}>Are you sure want to delete this user ?</h1>
          <button className={styles.submitButton} onClick={(id)=>handleDelete(id)}>Yes</button>
          <button className={styles.canelButton} onClick={()=>setIsOpen(false)}>Cancel</button>
        </div>
      </div>
    </div>
  );
